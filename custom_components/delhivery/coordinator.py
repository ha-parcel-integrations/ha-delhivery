"""Coordinator for the Delhivery parcel tracker integration.
...
from .api import DelhiveryApiClient, DelhiveryApiError
from .const import (
...
class DelhiveryCoordinator(DataUpdateCoordinator[list[dict]]):
    ...
    def __init__(
        ...
        # Tracking codes (AWBs) confirmed delivered on a prior refresh —
        # excluded from the fetch this cycle since a delivered parcel's
        # payload can never change again. Keyed on the AWB the request was
        # made with. Lives for the integration's lifetime (resets on
        # restart).
    ...
    async def _async_update_data(self) -> list[dict]:
        ...
        # (awb, raw) pairs — the AWB is always the code we requested, never
        # read from the payload (see parcels.normalize_parcel's docstring on
        # why: it is the only value guaranteed present, even on an unknown or
        # not-yet-scanned AWB).
        entries_raw: list[tuple[str, dict]] = []
        errors = 0
        for code, result in zip(codes_to_fetch, results):
            if isinstance(result, BaseException):
                if not isinstance(
                    result, (DelhiveryApiError, aiohttp.ClientError)
                ):
                    raise result
                errors += 1
                _LOGGER.warning("Delhivery fetch failed for %s: %s", code, result)
                cached = self._raw_cache.get(code)
                if cached is not None:
                    entries_raw.append((code, cached))
                continue

            if result is None:
                # Unknown AWB, or not scanned yet — the not-found envelope is
                # the only real signal this endpoint ever gives (§3 of the
                # mechanics doc). Keep prior data if we have it, otherwise
                # show a pending placeholder so the user still sees the
                # parcel they asked us to track.
                entries_raw.append((code, self._raw_cache.get(code) or {}))
                continue

            self._raw_cache[code] = result
            entries_raw.append((code, result))
        ...
        if codes_to_fetch and errors == len(codes_to_fetch) and not entries_raw:
            raise UpdateFailed("Delhivery unreachable for all tracked parcels")

        include_history = self._include_history
        entries = [
            (code, normalize_parcel(raw, awb=code, include_history=include_history))
            for code, raw in entries_raw
        ]
