    ...
    async def _async_update_data(self) -> list[dict]:
        ...
        for code, result in zip(codes, results):
            ...
            if result is None:
                ...
                entries.append((code, self._raw_cache.get(code) or {}))
                continue

            self._raw_cache[code] = result
            entries.append((code, result))

        if codes and errors == len(codes) and not entries:
            raise UpdateFailed("Delhivery unreachable for all tracked parcels")

        include_history = self._include_history
        normalized = [
            normalize_parcel(raw, awb=code, include_history=include_history)
