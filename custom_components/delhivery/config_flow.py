"""Config flow for the Delhivery parcel tracker integration."""
...
class DelhiveryConfigFlow(ConfigFlow, domain=DOMAIN):
    """Handle the UI-driven configuration flow for the Delhivery integration."""
    ...
    ) -> DelhiveryOptionsFlowHandler:
        """Return the options flow handler."""
        return DelhiveryOptionsFlowHandler()

    async def async_step_user(
        self, user_input: dict[str, Any] | None = None
    ) -> ConfigFlowResult:
        """Create the Delhivery hub — single instance, no input needed.

        Tracking is keyed on the AWB alone (no account, no postal code), so
        there is nothing to ask at setup: the entry is created straight away
        and parcels are added afterwards via the options flow, the
        ``delhivery.track_parcel`` service or a dashboard button.
...
class DelhiveryOptionsFlowHandler(OptionsFlow):
