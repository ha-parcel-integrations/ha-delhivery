"""Sensor platform for the Delhivery parcel tracker integration."""
...
from .coordinator import DelhiveryCoordinator
...
async def async_setup_entry(
    hass: HomeAssistant,
    entry: DelhiveryConfigEntry,
    async_add_entities: AddEntitiesCallback,
) -> None:
    """Set up Delhivery sensor entities from a config entry."""
    ...
    entities: list[SensorEntity] = [
        DelhiveryIncomingParcelsSensor(
            coordinator, entry, async_add_entities, current_barcodes
        ),
    ]
    for parcel in coordinator.data or []:
        entities.append(
            DelhiveryParcelSensor(coordinator, entry, parcel.get("barcode", ""))
        )
    entities.append(DelhiveryNextDeliverySensor(coordinator, entry))
    entities.append(DelhiveryDeliveredParcelsSensor(coordinator, entry))
    entities.append(DelhiveryLastUpdateSensor(coordinator, entry))

    async_add_entities(entities)


class DelhiveryIncomingParcelsSensor(
    CoordinatorEntity[DelhiveryCoordinator], SensorEntity
):
    ...
    def __init__(
        self,
        coordinator: DelhiveryCoordinator,
        entry: ConfigEntry,
        async_add_entities: AddEntitiesCallback,
    ...
    def _handle_coordinator_update(self) -> None:
        ...
        if new_barcodes:
            self._async_add_entities(
                DelhiveryParcelSensor(self.coordinator, self._entry, barcode)
...
class DelhiveryParcelSensor(CoordinatorEntity[DelhiveryCoordinator], SensorEntity):
    """Per-parcel sensor reporting the status of one tracked Delhivery parcel."""
    ...
    def __init__(
        self, coordinator: DelhiveryCoordinator, entry: ConfigEntry, barcode: str
    ) -> None:
        """Initialize the sensor."""
        super().__init__(coordinator)
        self._entry = entry
...
class DelhiveryNextDeliverySensor(
    CoordinatorEntity[DelhiveryCoordinator], SensorEntity
):
    ...
    def __init__(
        self, coordinator: DelhiveryCoordinator, entry: ConfigEntry
    ) -> None:
        """Initialize the sensor."""
        super().__init__(coordinator)
...
class DelhiveryDeliveredParcelsSensor(
    CoordinatorEntity[DelhiveryCoordinator], SensorEntity
):
    """Recently delivered tracked Delhivery parcels."""
...
class DelhiveryLastUpdateSensor(
    CoordinatorEntity[DelhiveryCoordinator], SensorEntity
):
    """Diagnostic sensor reporting when Delhivery was last polled successfully."""
