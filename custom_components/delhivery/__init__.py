"""Delhivery parcel tracker custom component for Home Assistant."""
...
from .api import DelhiveryApiClient
from .const import PLATFORMS
from .coordinator import DelhiveryCoordinator, _refresh_interval
...
@dataclass
class DelhiveryData:
    """Runtime data attached to the Delhivery config entry."""

    client: DelhiveryApiClient
    coordinator: DelhiveryCoordinator


type DelhiveryConfigEntry = ConfigEntry[DelhiveryData]


async def async_setup_entry(hass: HomeAssistant, entry: DelhiveryConfigEntry) -> bool:
    """Set up Delhivery from a config entry."""
    # No auth: Delhivery tracking is public, so the HA-managed session is fine.
    client = DelhiveryApiClient(async_get_clientsession(hass))
    ...
    entry.runtime_data = DelhiveryData(client=client, coordinator=coordinator)
...
async def async_unload_entry(hass: HomeAssistant, entry: DelhiveryConfigEntry) -> bool:
    """Unload the Delhivery config entry."""
