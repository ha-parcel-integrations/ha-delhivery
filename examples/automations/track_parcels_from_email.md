# Automatic parcel tracking from e-mail (IMAP → Delhivery)

Companion guide for [`track_parcels_from_email.yaml`](track_parcels_from_email.yaml): watch your mailbox(es) for shipping e-mails, extract the Delhivery tracking code, and register it with `delhivery.track_parcel` — fully automatic, no extra custom component required.

Delhivery is a **code-based** carrier: it has no account inbox, so every parcel must be registered by its tracking code before the integration can follow it. This recipe automates exactly that step.

**How it works, in one line:** the core [IMAP integration](https://www.home-assistant.io/integrations/imap/) fires an `imap_content` event for every new e-mail (including the body); the automation extracts the tracking code — a cheap regex first, an optional AI fallback for everything else — and calls `delhivery.track_parcel`.

```
new e-mail ──imap_content──▶ automation ──▶ regex match? ──▶ delhivery.track_parcel
                                     │
...
- This integration, with the `delhivery.track_parcel` action available (field `tracking_code`).
...
```
(?<![0-9A-Za-z])[0-9]{10,15}(?![0-9A-Za-z])
```

**This is a guess, not a confirmed format.** The only data point on the AWB
format is one 13-digit *bogus* probe number used to control-test the
endpoint — it proves the endpoint is numeric-AWB-shaped, nothing about the
real length range a genuine AWB uses
(`carrier-research/api/delhivery/tracking.md#tracking-code-format`, not
published in this public repo). The integration itself accepts any code
matching `^[A-Z0-9]{6,30}$` — deliberately loose for the same reason — but
matching *that* broadly against e-mail text would hit every order/invoice
number in the mail, which is why this example narrows to all-digits,
10–15 characters, instead. **If this misses (or over-matches) your real
AWBs, narrow or widen the `{10,15}` range** to what you actually see; the AI
fallback will surface anything the regex misses in the meantime.
...
1. **Jinja eats backslashes in string literals.** A template stored as `regex_findall('\b[0-9]{10,15}\b')` silently becomes a **backspace character** (`\b` is a string escape), so the regex never matches — no error anywhere. That's why the pattern is backslash-free: `(?<![0-9A-Za-z])` lookarounds instead of `\b`, `[0-9]` instead of `\d`. Copy that style if you narrow the length range.
...
curl -X POST -H "Authorization: Bearer $HA_TOKEN" -H "Content-Type: application/json" \
  ...
  -d '{"sender":"noreply@delhivery.com","subject":"Your parcel is on its way",
       "text":"AWB number: 1234567890123","initial":true,"folder":"INBOX","username":"test"}'
```

Then `delhivery.untrack_parcel` the test code afterwards. For a full end-to-end test, forward a real shipping mail to the watched mailbox — it must arrive **unread**.
