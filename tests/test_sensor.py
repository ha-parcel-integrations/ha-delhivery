"""Tests for Delhivery sensor property logic."""
from datetime import datetime, timezone
from unittest.mock import MagicMock

from custom_components.delhivery.const import ParcelStatus
from custom_components.delhivery.sensor import (
    DelhiveryDeliveredParcelsSensor,
    DelhiveryIncomingParcelsSensor,
    DelhiveryLastUpdateSensor,
    DelhiveryNextDeliverySensor,
    DelhiveryParcelSensor,
)
...
def test_incoming_counts_and_lists():
    ...
    sensor = DelhiveryIncomingParcelsSensor(coordinator, _entry(), lambda _: None, set())
...
def test_parcel_sensor_status_and_attributes():
    ...
    sensor = DelhiveryParcelSensor(_coordinator([parcel]), _entry(), "A")
...
def test_parcel_sensor_missing_barcode():
    sensor = DelhiveryParcelSensor(_coordinator([_parcel("A")]), _entry(), "OTHER")
...
def test_next_delivery_picks_earliest():
    ...
    sensor = DelhiveryNextDeliverySensor(coordinator, _entry())
...
def test_next_delivery_none_without_moments():
    sensor = DelhiveryNextDeliverySensor(_coordinator([_parcel("A")]), _entry())
    assert sensor.native_value is None
    assert sensor.extra_state_attributes == {}
...
def test_delivered_sensor():
    ...
    sensor = DelhiveryDeliveredParcelsSensor(coordinator, _entry())
...
def test_last_update_sensor():
    ...
    sensor = DelhiveryLastUpdateSensor(coordinator, _entry())
