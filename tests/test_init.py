"""Tests for Delhivery setup and unload."""
...
async def test_setup_and_unload(hass):
    ...
    with patch(
        "custom_components.delhivery.api.DelhiveryApiClient.async_get_parcel",
    ...
    incoming = hass.states.get("sensor.delhivery_incoming_parcels")
...
async def test_setup_retries_when_first_refresh_fails(hass):
    ...
    with patch(
        "custom_components.delhivery.api.DelhiveryApiClient.async_get_parcel",
        new=AsyncMock(side_effect=DelhiveryApiError("Delhivery unreachable")),
    ):
...
async def test_per_parcel_sensor_spawn_and_remove(hass):
    """barcode is always the tracked AWB, never read off the payload — so a
    new per-parcel sensor appears when the *tracked codes* change, not when
    the (identically-shaped) mocked payload does."""
    entry = MockConfigEntry(
        domain=DOMAIN,
        unique_id=DOMAIN,
        options={CONF_PARCELS: [{CONF_TRACKING_CODE: ACTIVE_CODE}]},
    ...
    with patch("custom_components.delhivery.api.DelhiveryApiClient.async_get_parcel", new=mock):
        assert await hass.config_entries.async_setup(entry.entry_id)
        await hass.async_block_till_done()
        ...
        # Swap which AWB is tracked: the summary sensor spawns a new
